...
function App() {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>(() => {
    ...
    return saved ? JSON.parse(saved) : [{
      id: 'welcome',
      role: 'model',
      text: `Hello CEO ${userProfile.name ? userProfile.name : ''}. AntiRisk Intelligence Vault is active.`,
      timestamp: Date.now()
    }];
  });
  ...
  // Derived State (Hooks moved to top level to avoid Error #310)
  ...
  const handleError = (error: any) => {
    setApiError(`Link Failure: ${error?.message || "Internal error."}`);
  };
  ...
  const renderDashboard = () => {
    const menuItems = [
      { id: View.ADVISOR, label: 'AI Executive Advisor', desc: 'Direct strategic security consultation.', icon: ShieldAlert, color: 'text-blue-400', bg: 'bg-blue-400/10' },
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-slate-400 font-medium text-sm sm:text-base">Automated weekly curriculum for guards and supervisors.</p>
            </div>
          </div>
  ...
  const renderTrainingView = () => {
    ...
    return (
      <div className="max-w-6xl mx-auto flex flex-col lg:flex-row gap-8 animate-in fade-in h-full">
        <div className="w-full lg:w-[380px] shrink-0">
          <div className="bg-[#0f172a] border border-slate-800 rounded-[2rem] p-8 shadow-2xl flex flex-col h-fit">
            <div className="flex items-center justify-between mb-2">
              ...
              <Wand2 className="text-emerald-400/50" size={20} />
            </div>
            ...
            {messages.map(msg => (
              <div key={msg.id} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                <div className={`group relative max-w-[85%] p-5 rounded-2xl ${msg.role === 'user' ? 'bg-blue-600 text-white rounded-br-none' : 'bg-slate-800 text-slate-100 rounded-bl-none border border-slate-700 shadow-sm'}`}>
                  {msg.role === 'model' && msg.text && <button onClick={() => togglePinMessage(msg.id)} className={`absolute -right-8 top-0 p-2 transition-all opacity-0 group-hover:opacity-100 ${msg.isPinned ? 'text-yellow-400 opacity-100' : 'text-slate-600'}`}>{msg.isPinned ? <BookmarkCheck size={18} /> : <Bookmark size={18} />}</button>}
  ...
  const renderBestPractices = () => (
    <div className="max-w-6xl mx-auto space-y-10 animate-in fade-in">
      <div className="bg-[#1b2537] p-12 rounded-[3rem] border border-slate-700/50 shadow-xl flex flex-col md:flex-row justify-between items-center gap-10">
        <div className="space-y-4">
          ...
          <p className="text-slate-400 text-lg font-medium">Real-time ISO standards monitoring and market intelligence shift analysis.</p>
        </div>
